...
1. 添加 ESA 站点，按提示把域名解析改为 ESA 提供的 CNAME。
2. 回源地址填服务器公网 IP，回源 HOST 填你的域名，端口 443。
3. **缓存规则**：所有路径、所有文件类型设为「不缓存」，防止私人内容被缓存泄露。

> 系统所有 PHP 页面已自动返回 `Cache-Control: no-store`，但建议在 ESA 里再明确配置不缓存，双保险。
